...
import { baclendUrl, currency } from "../App";
...
const List = ({token}) => {
  ...
  const fectchList = async () => {
    try {
      const response = await axios.get(baclendUrl + "/api/product/list");
  ...
  const removeProduct = async(id)=>{
    try {
      ...
      const response = await axios.post(baclendUrl + "/api/product/remove", {id}, {headers:{token}});
      if(response.data.success){
